...
export default {
    ...
    methods: {
        ...
        load(page, search) {
            ...
            this.search = search
            axios.get('users?page=' + this.paginations.nextpage + '&s=' + this.search).then(res => {
        ...
        add() {
            axios.post('users', this.form).then(res => {
                this.success = true
                //untuk hide modal 
                const $elementadd = document.getElementById('add-user-modal');
                const modal = new Modal($elementadd, modalOptions);
                modal.hide()
                this.load('', '')
            }).catch((err) => {
                // jika ada validasi dari api
                this.allerros = err.response.data.errors;
                this.success = false
        ...
        edit(user) {
            const $elementadd = document.getElementById('edit-user-modal');
        ...
        deleteV(user) {
            //untuk menampilkan modal 
            const $elementadd = document.getElementById('delete-user-modal');
            const modal = new Modal($elementadd, modalOptions);
            modal.show()
            this.form.id = user.id
        },
        ...
        // update data users
        update(id) {
            return axios.put('users/' + id, { name: this.form.name, role: this.form.role }).then(res => {
                this.success = true
                //untuk hide modal
                const $elementadd = document.getElementById('edit-user-modal');
                const modal = new Modal($elementadd, modalOptions);
            ...
            <div class="sm:flex">
                <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700">
                    ...
                    <div class="relative mt-1 lg:w-64 xl:w-96">
                        <input type="text" @change="searchF(this.search)" name="search" v-model="search" id="users-search"
                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700"
                            v-for="user in users" :key="user.idd">
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    <button type="button" @click="edit(user)"
                                    ...
                                    <button type="button" @click="deleteV(user)"
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <input type="text" v-model="form.name" name="name" id="name"
                                    ...
                                    placeholder="Bonnie">
                                <span v-if="allerros.name" :class="['mt-2 text-sm text-red-600 dark:text-red-500']">{{
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <label for="role"
                                    ...
                                    Position</label>
                                <select id="countries" v-model="form.role"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    <option value="" selected>Choose a position</option>
                                    <option value="admin">Admin</option>
                                    <option value="user">User</option>
                                </select>
                                <span v-if="allerros.role" :class="['mt-2 text-sm text-red-600 dark:text-red-500']">{{
                                    allerros.role[0] }}</span>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="email"
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <select id="role" v-model="form.role"
                                    ...
                                    <option value="user" :selected="form.role == 'manager'">User</option>
                                </select>
                                <span v-if="allerros.role" :class="['mt-2 text-sm text-red-600 dark:text-red-500']">{{
                                    allerros.role[0] }}</span>
                            </div>
